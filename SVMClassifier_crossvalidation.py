...
# Cross-Validation
cv_accuracy = cross_val_score(svm_classifier, X_train, y_train, cv=5, scoring='accuracy')
cv_precision = cross_val_score(svm_classifier, X_train, y_train, cv=5, scoring='precision_weighted')
cv_recall = cross_val_score(svm_classifier, X_train, y_train, cv=5, scoring='recall_weighted')
cv_f1 = cross_val_score(svm_classifier, X_train, y_train, cv=5, scoring='f1_weighted')

# Fit the SVM Classifier on the entire training data
...
def show_model_performance():
    ...
    performance_window.title("SVM Classifier Performance Metrics")
    performance_window.geometry("500x400")
    performance_window.configure(bg="#e0f7fa")

    Label(performance_window, text="SVM Classifier Performance Metrics", font=("Helvetica", 14), bg="#e0f7fa").pack(pady=10)
...
def predict():
    try:
        ...
        predicted_outcome_encoded = svm_classifier.predict(input_data)[0]
