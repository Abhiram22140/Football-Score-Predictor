...
lr_home.fit(X_train, y_train_home)
lr_away.fit(X_train, y_train_away)

# Evaluate Model Performance
y_pred_home = lr_home.predict(X_test)
y_pred_away = lr_away.predict(X_test)
...
def show_model_performance():
    ...
    Label(performance_window, text="Linear Regression Model Performance Metrics", font=("Helvetica", 14), bg="#e0f7fa").pack(pady=10)
    ...
    # Feature Coefficients Plot (as feature importance for Linear Regression)
    fig, ax = plt.subplots(figsize=(8, 6))
    feature_coefficients = lr_home.coef_
    features = X.columns
    sns.barplot(x=feature_coefficients, y=features, ax=ax, palette="viridis")
    ax.set_title("Feature Coefficients for Home Goals Prediction")
...
def predict():
    try:
        ...
        # Predict scores
        predicted_home_goals = round(lr_home.predict(input_data)[0])
        predicted_away_goals = round(lr_away.predict(input_data)[0])
...
# Show Model Performance button (continued)
