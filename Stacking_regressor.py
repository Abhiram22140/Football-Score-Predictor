...
# Define base models and meta-regressor for stacking
base_models = [
...
# Create Stacking Regressors for Home and Away Goals
stacking_home = StackingRegressor(estimators=base_models, final_estimator=meta_regressor, n_jobs=-1)
stacking_away = StackingRegressor(estimators=base_models, final_estimator=meta_regressor, n_jobs=-1)

# Train Stacking Models
stacking_home.fit(X_train, y_train_home)
stacking_away.fit(X_train, y_train_away)

# Evaluate Model Performance
y_pred_home = stacking_home.predict(X_test)
y_pred_away = stacking_away.predict(X_test)
...
# Display Model Performance in New Window
def show_model_performance():
    performance_window = Toplevel(root)
    performance_window.title("Stacking Regressor Performance Metrics")
    performance_window.geometry("500x400")
    performance_window.configure(bg="#e0f7fa")

    Label(performance_window, text="Stacking Regressor Model Performance Metrics", font=("Helvetica", 14), bg="#e0f7fa").pack(pady=10)
...
def predict():
    try:
        ...
        # Predict scores
        predicted_home_goals = round(stacking_home.predict(input_data)[0])
        predicted_away_goals = round(stacking_away.predict(input_data)[0])
