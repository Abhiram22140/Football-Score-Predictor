...
# Adjusting the Bi-LSTM model architecture
model = Sequential()
model.add(Bidirectional(LSTM(units=128, activation='tanh', return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2]))))
...
y_pred = np.nan_to_num(y_pred)  # Handle potential NaN values in predictions
