...
# Train Gradient Boosting Models for Home and Away Goals
...
y_pred_home = gb_home.predict(X_test)
y_pred_away = gb_away.predict(X_test)
...
def show_model_performance():
    ...
    Label(performance_window, text="Gradient Boosting Model Performance Metrics", font=("Helvetica", 14), bg="#e0f7fa").pack(pady=10)
    ...
    feature_importances = gb_home.feature_importances_
    features = X.columns
...
def predict():
    try:
        ...
        # Predict scores
        predicted_home_goals = round(gb_home.predict(input_data)[0])
        predicted_away_goals = round(gb_away.predict(input_data)[0])
