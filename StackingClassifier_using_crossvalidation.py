...
# Define base models and meta-classifier for stacking
base_models = [
...
stacking_classifier = StackingClassifier(estimators=base_models, final_estimator=meta_classifier, n_jobs=-1)

# Cross-Validation
cv_accuracy = cross_val_score(stacking_classifier, X_train, y_train, cv=5, scoring='accuracy')
cv_precision = cross_val_score(stacking_classifier, X_train, y_train, cv=5, scoring='precision_weighted')
cv_recall = cross_val_score(stacking_classifier, X_train, y_train, cv=5, scoring='recall_weighted')
cv_f1 = cross_val_score(stacking_classifier, X_train, y_train, cv=5, scoring='f1_weighted')
...
def show_model_performance():
    ...
    performance_window.title("Stacking Classifier Performance Metrics")
    performance_window.geometry("500x400")
    performance_window.configure(bg="#e0f7fa")

    Label(performance_window, text="Stacking Classifier Performance Metrics", font=("Helvetica", 14), bg="#e0f7fa").pack(pady=10)
...
def predict():
    try:
        ...
        predicted_outcome_encoded = stacking_classifier.predict(input_data)[0]
