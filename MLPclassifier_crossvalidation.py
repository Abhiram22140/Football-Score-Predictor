...
mlp_classifier = MLPClassifier(hidden_layer_sizes=(50, 30), max_iter=300, random_state=42)

# Cross-Validation
cv_accuracy = cross_val_score(mlp_classifier, X_train, y_train, cv=5, scoring='accuracy')
cv_precision = cross_val_score(mlp_classifier, X_train, y_train, cv=5, scoring='precision_weighted')
cv_recall = cross_val_score(mlp_classifier, X_train, y_train, cv=5, scoring='recall_weighted')
cv_f1 = cross_val_score(mlp_classifier, X_train, y_train, cv=5, scoring='f1_weighted')

# Fit the MLP Classifier on the entire training data
...
def show_model_performance():
    ...
    performance_window.title("MLP Classifier Performance Metrics")
    performance_window.geometry("500x400")
    performance_window.configure(bg="#e0f7fa")

    Label(performance_window, text="MLP Classifier Performance Metrics", font=("Helvetica", 14), bg="#e0f7fa").pack(pady=10)
...
def predict():
    try:
        ...
        predicted_outcome_encoded = mlp_classifier.predict(input_data)[0]
